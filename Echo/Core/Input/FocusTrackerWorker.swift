...
/// (blocking IPC to another app) never runs on the main thread, where Echo's keyboard event tap
...
final class FocusTrackerWorker: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.rhysellwood.echo.focus-tracker", qos: .userInitiated)
    ...
    /// Apps Echo switched AXManualAccessibility on in, to switch back off when tracking stops.
    ...
    /// Detaches and switches AXManualAccessibility back off wherever Echo switched it on. With a
    /// timeout, waits up to that long for it (when Echo is quitting and won't get another chance).
