...
enum DictationPresentation {
    ...
    static func tint(for state: DictationState) -> Color {
        switch state {
        case .idle: return EchoColor.inkMuted
        case .preparing, .transcribing: return EchoColor.accent
        case .listening: return EchoColor.live
        case .inserting, .failed(.microphoneReady): return EchoColor.success
        case .failed: return EchoColor.warning
        }
    }
    ...
    static func hudSubtitle(for state: DictationState, isModelLoading: Bool) -> String? {
        ...
        case .failed(let failure):
            ...
            case .microphoneDenied, .accessibilityDenied: return "Allow it in Echo › Permissions"
            case .noModelInstalled: return "Choose one from Echo's menu"
            case .modelStillLoading: return "Try again in a moment"
            case .modelLoadFailed, .audioFailed, .transcriptionFailed: return "Details are in Echo's menu"
