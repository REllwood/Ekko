...
struct FieldMicView: View {
    ...
    private var face: some View {
        if state.isListening {
            ...
            Circle()
                .fill(model.isHovering ? EchoColor.accent.opacity(0.14) : Color.clear)
                .echoGlassBackground(in: Circle())
                .overlay(
                    Circle().strokeBorder(
                        model.isHovering ? EchoColor.accent.opacity(0.6) : EchoColor.hairlineStrong,
                        lineWidth: 1
                    )
                )
    ...
    private var glyph: some View {
        ...
        case .preparing, .transcribing:
            ProgressView()
                ...
                .controlSize(.mini)
        case .inserting:
            symbol("checkmark", color: EchoColor.success, weight: .bold)
        case .failed:
            symbol(DictationPresentation.glyph(for: state), color: EchoColor.warning)
        case .listening:
            // Hovering a live mic previews what a click does.
            symbol(model.isHovering ? "stop.fill" : "mic.fill", color: .white)
        case .idle:
            symbol("mic.fill", color: model.isHovering ? EchoColor.accent : EchoColor.inkSoft)
        }
    }
...
private struct FieldMicPulse: View {
    ...
    var body: some View {
        ...
        Circle()
            .fill(EchoColor.live.opacity(reduceMotion ? 0.22 : 0.16 + 0.2 * Double(level)))
