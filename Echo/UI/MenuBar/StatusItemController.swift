...
final class StatusItemController: NSObject, NSPopoverDelegate {
    ...
    /// The app that was frontmost before the popover activated Echo.
    ...
    func install() {
        ...
        if let button = item.button {
            ...
            button.setAccessibilityLabel("Echo dictation")
            button.toolTip = "Echo — press \(container.settings.hotkey.displayString) to dictate"
        }
        statusItem = item
    ...
    private func togglePopover(from button: NSStatusBarButton) {
        if popover.isShown {
            ...
            // Only remember a real other app; if Echo is already in front there is nothing to return to.
    ...
    private func showMenu() {
        ...
        let aboutItem = NSMenuItem(title: "About Echo", action: #selector(openAbout), keyEquivalent: "")
        aboutItem.target = self
        menu.addItem(aboutItem)

        menu.addItem(.separator())

        let quitItem = NSMenuItem(title: "Quit Echo", action: #selector(quit), keyEquivalent: "q")
    ...
    /// The popover activated Echo, so the user's text field lost focus. Close the popover,
    ...
    /// Opening the popover made Echo the active app. If it was dismissed with Esc or a second
    /// click on the icon, Echo would stay in front with no window, and the next dictation would
    ...
    private func refreshState() {
        ...
        statusItem?.button?.toolTip = "Echo — press \(container.settings.hotkey.displayString) to dictate"
    ...
    private func updateIcon() {
        ...
        switch displayedState {
        case .idle:
            button.image = StatusItemIcon.symbol("waveform")
        case .preparing, .transcribing:
            button.image = StatusItemIcon.transcribing(frame: frameIndex)
        case .listening:
            button.image = StatusItemIcon.listening(frame: frameIndex)
        case .inserting:
            button.image = StatusItemIcon.symbol("checkmark")
        case .failed:
            button.image = StatusItemIcon.symbol("exclamationmark.triangle")
        }
        button.setAccessibilityLabel("Echo — \(DictationPresentation.title(for: displayedState))")
    }
}
