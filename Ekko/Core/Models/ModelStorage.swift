...
///
///     ~/Library/Application Support/Ekko/Models/models/argmaxinc/whisperkit-coreml/<variant>/
///
...
enum ModelStorage {
    ...
    /// `~/Library/Application Support/Ekko/Models`.
    static var defaultDownloadBase: URL {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Library/Application Support", isDirectory: true)
        return support.appendingPathComponent("Ekko/Models", isDirectory: true)
    }
