...
/// the main thread, which also runs Ekko's keyboard event tap. This side only decides when to read
...
final class FocusTracker {
    ...
    /// True while the focused field's text keeps changing (typing, or Ekko inserting). Clears about
    /// `typingWindow` after the last change.
    private(set) var isUserTyping = false

    static let pollInterval: TimeInterval = 0.5
    static let typingWindow: TimeInterval = 0.8
    /// How long quitting Ekko waits for AXManualAccessibility to be switched back off.
    ...
    /// Stops following focus and switches AXManualAccessibility back off in the apps Ekko set it in.
    func stop() {
