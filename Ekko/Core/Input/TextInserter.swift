...
enum InsertionError: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .targetIsSelf: return "Cannot insert into Ekko itself."
...
final class TextInserter {
    ...
    static let markerType = NSPasteboard.PasteboardType("com.rhysellwood.ekko.session")
    ...
    /// Ekko can end up frontmost mid-dictation (the menu-bar popover activates it). The text must
    /// go to the app the user was dictating into, so hand focus back to it first. Dictating into
    /// Ekko's own window (the onboarding practice box) is left alone.
    @MainActor
    private func returnFocusToTarget(of context: FocusContext) async {
        ...
        Log.input.info("Ekko is frontmost; returning focus to \(app.localizedName ?? "the target app", privacy: .public)")
        app.activate()
