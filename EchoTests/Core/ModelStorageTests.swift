...
final class ModelStorageTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        root = FileManager.default.temporaryDirectory
            .appendingPathComponent("EchoModelStorageTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testVariantFolderMatchesWhisperKitsHubLayout() {
        let base = URL(fileURLWithPath: "/tmp/Echo/Models", isDirectory: true)
        let folder = ModelStorage.folder(downloadBase: base, variant: "openai_whisper-base")
        XCTAssertEqual(
            folder.path,
            "/tmp/Echo/Models/models/argmaxinc/whisperkit-coreml/openai_whisper-base"
        )
        XCTAssertEqual(
            ModelStorage.variantsRoot(downloadBase: base).path,
            "/tmp/Echo/Models/models/argmaxinc/whisperkit-coreml"
        )
    }

    func testDefaultDownloadBaseIsInApplicationSupport() {
        let base = ModelStorage.defaultDownloadBase
        XCTAssertTrue(base.path.hasSuffix("Application Support/Echo/Models"), base.path)
    }
